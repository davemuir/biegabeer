...
define('WP_TWITTER_RETWEET_BUTTON_VERSION', '1.15');
...
function wp_twitterbutton() {
	if(wp_twitter_get_option('wp_twitter_where') == 'manual') {
		echo wp_twitter_generate_button();
	} else {
		return false;
	}
}
				...
				<tr>
					...
					<td>
						<div style="float:right; text-align:center; width:120px;">
							...
							<a href="http://flattr.com/thing/79727/WP-Twitter-Retweet-Button" target="_blank"><img src="http://api.flattr.com/button/flattr-badge-large.png" alt="Flattr this" title="Flattr this" border="0" /></a>
						</div>
						<div>
 ...
 * @since 1.9.0
 */
if(!function_exists('wp_twitter_update_notice')) {
	function wp_twitter_update_notice() {
		$url = 'http://plugins.trac.wordpress.org/browser/wp-twitter-retweet-button/trunk/readme.txt?format=txt';
		$data = '';
		...
		if($data) {
			$matches = null;
			$regexp = '~==\s*Changelog\s*==\s*=\s*[0-9.]+\s*=(.*)(=\s*' . preg_quote(WP_TWITTER_RETWEET_BUTTON_VERSION) . '\s*=|$)~Uis';
			...
			if(preg_match($regexp, $data, $matches)) {
				...
				foreach($changelog as $index => $line) {
					if(version_compare($version, WP_TWITTER_RETWEET_BUTTON_VERSION,">")) {
					...
					} // END if(version_compare($version, WP_TWITTER_RETWEET_BUTTON_VERSION,">"))
	...
	} // END function wp_twitter_update_notice()
} // END if(!function_exists('wp_twitter_update_notice'))
...
if(is_admin()) {
	...
	if(ini_get('allow_url_fopen') || function_exists('curl_init')) {
		add_action('in_plugin_update_message-' . plugin_basename(__FILE__), 'wp_twitter_update_notice');
	}
}
