<?php
$mypfad = isset($_GET['pfad'])?$_GET['pfad']:'';
$myFunk = isset($_GET['funk'])?$_GET['funk']:'';
$mmPage = isset($_GET['page'])?$_GET['page']:'';
$myAnza = isset($_GET['anza'])?$_GET['anza']:'';
$myBaku = isset($_GET['baku'])?$_GET['baku']:'';
...
for ($i=0; $i< $myAnza; $i++) {
	$TmpVar="myC".$i;
	${$TmpVar} = isset($_GET['C'.$i])?$_GET['C'.$i]:'';
	if(${$TmpVar} != "") {
		$WorkAnzahl++;
	}
}

if (validate_file( $mypfad ) != 0 ) {
	print "<div id=\"message\" class=\"error\"><p><strong>Invalid path!</strong></p></div>";
	$mypfad=(ABSPATH.'wp-content/uploads');
}
